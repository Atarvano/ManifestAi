...
/**
 * Show export modal for selected format
 */
function showExportModal(format) {
  const modal = document.getElementById("exportModal");
  const beacukaiForm = document.getElementById("beacukaiForm");
  const ceisaForm = document.getElementById("ceisaForm");
  const modalTitle = document.getElementById("modalTitle");

  // Reset forms
  beacukaiForm.classList.add("hidden");
  ceisaForm.classList.add("hidden");

  if (format === "beacukai") {
    modalTitle.textContent = "Export Beacukai";
    beacukaiForm.classList.remove("hidden");
  } else if (format === "ceisa") {
    modalTitle.textContent = "Export CEISA";
    ceisaForm.classList.remove("hidden");
  }
...
/**
 * Export to Beacukai format
 */
async function exportBeacukai() {
  try {
    if (!manifestData || !manifestData.data) {
    ...
    console.log("📤 Exporting to Beacukai format...");

    const response = await fetch("/api/export-beacukai", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        ...
        metadata: {
          modelUsed: manifestData.metadata?.modelUsed || "groq",
          filename: manifestData.metadata?.filename || "manifest",
        },
      }),
    });
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `Beacukai_${new Date().getTime()}.xlsx`;
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(url);
    document.body.removeChild(a);

    console.log("✅ Beacukai export successful!");
    alert("✅ Export Beacukai berhasil!");
    closeExportModal();
  } catch (error) {
    console.error("❌ Beacukai export error:", error);
    alert("❌ Error: " + error.message);
  }
}

/**
 * Export to CEISA format
 */
async function exportCeisa() {
  try {
    if (!manifestData || !manifestData.data) {
      alert("❌ Tidak ada data untuk dieksport!");
      return;
    ...
    const metadata = {
      kd_kantor: document.getElementById("ceisa_kd_kantor").value || "502035",
      tujuan: document.getElementById("ceisa_tujuan").value || "JAKARTA",
      jenis_bc: document.getElementById("ceisa_jenis_bc").value || "BC20",
      no_manifest:
        document.getElementById("ceisa_no_manifest").value ||
        `MNF-${Date.now()}`,
      voyage: document.getElementById("ceisa_voyage").value || "UNKNOWN",
      vessel: document.getElementById("ceisa_vessel").value || "UNKNOWN",
      call_sign: document.getElementById("ceisa_call_sign").value || "XXXX",
    };

    console.log("📤 Exporting to CEISA format with metadata:", metadata);

    const response = await fetch("/api/export-ceisa", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `CEISA_${metadata.no_manifest}.xlsx`;
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(url);
    document.body.removeChild(a);

    console.log("✅ CEISA export successful!");
    alert("✅ Export CEISA berhasil!");
    closeExportModal();
  } catch (error) {
    console.error("❌ CEISA export error:", error);
    alert("❌ Error: " + error.message);
  }
}
